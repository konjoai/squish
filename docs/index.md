...
**Squish wins end-to-end response time by 9.8× at 4000-token prompts**, uses less RAM (3.50 vs 5.14 GB), and supports INT3 quantization for compatible model families with accuracy within measurement noise.

**Ollama wins cold short-prompt TTFT** (167 ms vs 192 ms). If first-byte latency on a cold, novel short prompt matters most, that 25 ms goes to Ollama. Squish leads everywhere else.
...
- **Long-context wins**: 9.8× faster end-to-end response at 4000-token prompts vs Ollama 0.30.7
