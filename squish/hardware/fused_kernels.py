...
Usage
-----
    from squish.hardware.fused_kernels import FusedAttention, FusedFFNGate, patch_model
...
// fused_sdpa — squish.hardware.fused_kernels
...
def patch_model_compiled_ffn(model, *, metal_fusion_kernels=None) -> int:
    ...
    gate activation through :func:`squish.hardware.metal_fusion.fused_swiglu`.  When
    ...
    metal_fusion_kernels : MetalFusionKernels | None
        Live kernel manager from :mod:`squish.hardware.metal_fusion`.  Pass ``None``
    ...
    if use_metal:  # pragma: no cover
        try:
            from squish.hardware.metal_fusion import fused_swiglu as _fused_swiglu  # noqa: PLC0415
            patched = 0
            for layer in getattr(model, "layers", []):
                mlp = getattr(layer, "mlp", None)
                if mlp is None:
