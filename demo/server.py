...
# These are validation bounds for the public planner endpoint. They are
# wider than the live-codec limits above because /api/recommend does
# closed-form math only — no token loop, so larger values stay cheap.
GET_REC_MAX_MODEL_B    = 200    # upper bound on model_size_b
GET_REC_MAX_CTX_LEN    = 1_000_000
GET_REC_MAX_BUDGET_MB  = 1_000_000   # 1 TB — sky-high; just guards int parsing

# Closed-form architecture table for the GET /api/recommend planner.
# Source: published Qwen2.5/Llama configs. The planner snaps the
# `model_size_b` query parameter to the nearest entry by absolute
# difference; ties go to the larger model (the conservative choice for
# memory planning). Adding a row is the one-liner way to support a new
# model size without changing planner code.
