#!/usr/bin/env python3
"""Run 500-sample eval: FP16 attn + embed + MLP (LN still INT4)."""
import json
import sys
...
COMPRESSED_DIR = str(Path.home() / "models" / "Qwen2.5-1.5B-Instruct-squished-fp16mlp")
MODEL_ARGS     = f"model_dir={MODEL_DIR},compressed_dir={COMPRESSED_DIR}"
RESULTS_DIR    = ROOT / "dev" / "results"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

print(f"\n{'='*60}")
print("  FP16 attn+embed+mlp, LN INT4 — 500-sample eval")
...
out = {
    "model": "Qwen2.5-1.5B-FP16attn-embed-mlp-noAWQ-LNint4",
    "compressed_dir": COMPRESSED_DIR, "limit": 500, "num_fewshot": 0,
    "results": summary, "all_beat_bf16": all_beat,
}
out_path = RESULTS_DIR / "accuracy_fp16mlp_500.json"
