#!/usr/bin/env python3
"""Run 500-sample eval against the fully-lossless squish passthrough model.

If this matches BF16 reference (~0.635 hellaswag), then weight quantization
is the bottleneck. If it still shows ~0.612, there is a systematic difference
in squish inference path vs mlx_lm direct inference.
"""
import json
import sys
...
COMPRESSED_DIR = str(Path.home() / "models" / "Qwen2.5-1.5B-Instruct-squished-lossless")
MODEL_ARGS     = f"model_dir={MODEL_DIR},compressed_dir={COMPRESSED_DIR}"
RESULTS_DIR    = ROOT / "dev" / "results"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

print(f"\n{'='*60}")
print("  LOSSLESS (BF16→FP16→BF16 only) — 500-sample eval")
...
out = {
    "model": "Qwen2.5-1.5B-lossless-FP16passthrough",
    "compressed_dir": COMPRESSED_DIR, "limit": 500, "num_fewshot": 0,
    "results": summary, "all_beat_bf16": all_beat,
}
out_path = RESULTS_DIR / "accuracy_lossless_500.json"
