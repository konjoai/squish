#!/usr/bin/env python3
"""Run 500-sample accuracy eval against the mixed-precision compressed model.

Mixed precision: FP16 self_attn + INT4 g=16 AWQ MLP.
"""
import json
import sys
...
COMPRESSED_DIR = str(Path.home() / "models" / "Qwen2.5-1.5B-Instruct-squished-mixed")
MODEL_ARGS     = f"model_dir={MODEL_DIR},compressed_dir={COMPRESSED_DIR}"
RESULTS_DIR    = ROOT / "dev" / "results"
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

print(f"\n{'='*60}")
print("  Mixed-precision (FP16 attn + INT4 g=16 AWQ MLP) — 500-sample eval")
...
out = {
    "model":          "Qwen2.5-1.5B-mixed-FP16attn-INT4MLP-AWQ",
...
out_path = RESULTS_DIR / "accuracy_mixed_precision_500.json"
