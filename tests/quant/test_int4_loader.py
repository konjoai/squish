...
def test_int4_conversion_and_round_trip():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # group_size=32 uses 2× as many scale values as group_size=64; and
        # __z4a offsets are now float32 (4 bytes/group) instead of uint8 (1 byte/group),
        # reducing raw byte savings from ~50% to ~26%.  The accuracy improvement is worth it.
        assert 20 < savings < 60, f"Expected ~25-35% savings, got {savings:.1f}%"
