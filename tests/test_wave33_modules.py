...
class TestFusedQKVProjection:
    ...
    def test_project_matches_separate_matmuls(self):
        ...
        k_ref = x @ w_k
        v_ref = x @ w_v
        # atol=3e-5 accounts for float32 accumulation differences between
        ...
        np.testing.assert_allclose(q_fused, q_ref, atol=3e-5)
        np.testing.assert_allclose(k_fused, k_ref, atol=3e-5)
        np.testing.assert_allclose(v_fused, v_ref, atol=3e-5)
