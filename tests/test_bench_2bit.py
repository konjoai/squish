...
_BENCH_PATH = Path(__file__).resolve().parents[1] / "dev" / "benchmarks" / "bench_2bit.py"
...
class TestBenchVptq:
    ...
    def test_skip_when_vptq_unavailable(self):
        # Temporarily hide squish.vptq.
        with patch.dict(sys.modules, {"squish.vptq": None}):
            r = bench_2bit.bench_vptq(_W_SMALL)
        assert r.status == "skip"
...
class TestBenchQuip:
    def test_ok_with_real_module(self):
        """squish.quip_sharp is implemented (Phase 9B done) → result is ok."""
    ...
    def test_skip_when_module_import_fails(self):
        """When squish.quip_sharp raises ImportError, bench_quip returns SKIP."""
        with patch.dict(sys.modules, {"squish.quip_sharp": None}):
            r = bench_2bit.bench_quip(_W_SMALL)
        assert r.status == "skip"
    ...
    def test_ok_when_module_mocked(self):
        ...
        fake_quip = types.ModuleType("squish.quip_sharp")
        ...
        with patch.dict(sys.modules, {"squish.quip_sharp": fake_quip}):
            r = bench_2bit.bench_quip(_W_SMALL)
