...
import squish.hardware.fused_kernels as fk
from squish.hardware.fused_kernels import FusedAttention, FusedFFNGate
...
class TestModuleLevelImportError:
    def test_mlx_unavailable_sets_flags_false(self):
        """Reload squish.hardware.fused_kernels with mlx mocked as None → covers lines 56-57."""
        saved_mlx_core = sys.modules.get("mlx.core")
        saved_mlx_nn   = sys.modules.get("mlx.nn")
        saved_fk       = sys.modules.pop("squish.hardware.fused_kernels")

        sys.modules["mlx.core"] = None
        sys.modules["mlx.nn"]   = None
        try:
            mod = importlib.import_module("squish.hardware.fused_kernels")
            ...
            # Restore the original squish.hardware.fused_kernels module
            sys.modules["squish.hardware.fused_kernels"] = saved_fk
