...
_METAL_FUSION_AVAILABLE
  - is a bool
  - importable from squish.hardware.fused_kernels
...
from squish.hardware.fused_kernels import (
    _METAL_FUSION_AVAILABLE,
    patch_model_compiled_ffn,
)
...
class TestMetalFusionAvailableSentinel:
    ...
    def test_value_consistent_with_metal_fusion_module(self):
        ...
        try:
            from squish.hardware.metal_fusion import _METAL_FUSION_AVAILABLE as _mf_val
...
class TestPatchModelCompiledFFN:
    ...
    def test_swiglu_disabled_uses_patch_model_fallback(self):
        ...
        model = _empty_model()
        with patch("squish.hardware.fused_kernels.patch_model", return_value=0) as mock_pm:
    ...
    def test_delegate_fallback_call_signature(self):
        ...
        model = _empty_model()
        sentinel = 99
        with patch("squish.hardware.fused_kernels.patch_model", return_value=sentinel) as mock_pm:
