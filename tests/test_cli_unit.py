...
# Regression: `squish run gemma3:4b` failed because pull creates only the
# `<base>-int4` dir, while _resolve_model required the raw `<base>-bf16` dir to
# exist first and died before computing the compressed dir.
...
class TestResolveModelCompressedFallback:
    ...
    def test_falls_back_to_int4_when_bf16_dir_absent(self, tmp_path):
        ...
        cli = _import_cli()
        int4 = self._make_quant_dir(tmp_path, "gemma-3-4b-it-int4")
        with (
            patch.object(cli, "_MODELS_DIR", tmp_path),
            patch.object(cli, "_CATALOG_AVAILABLE", True),
            patch.object(cli, "_catalog_resolve", return_value=self._entry("gemma-3-4b-it-bf16")),
        ):
    ...
    def test_honors_requested_quant_mode(self, tmp_path):
        ...
        cli = _import_cli()
        self._make_quant_dir(tmp_path, "gemma-3-4b-it-int4")
        int3 = self._make_quant_dir(tmp_path, "gemma-3-4b-it-int3")
        with (
            patch.object(cli, "_MODELS_DIR", tmp_path),
            patch.object(cli, "_CATALOG_AVAILABLE", True),
            patch.object(cli, "_catalog_resolve", return_value=self._entry("gemma-3-4b-it-bf16")),
