...
def test_line_count():
    """server.py must be ≤ 4780 lines (Wave 125 target 4702; squash routing +41;
    W111 quality monitor endpoint +18 — all exempt from purge targets)."""
    count = len(LINES)
    assert count <= 4780 and count > 4600, (
        f"Expected ≤ 4780 lines (W111 quality monitor adjusted), got {count}"
    )
