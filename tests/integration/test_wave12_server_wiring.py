...
class TestPMKVQWiring:
    def test_import(self):
        from squish.kv.pm_kvq import PMKVQConfig, PMKVQScheduler
...
class TestMixKVQWiring:
    def test_import(self):
        from squish.kv.mix_kvq import MixKVQConfig, MixKVQQuantizer
        cfg  = MixKVQConfig()
        quant = MixKVQQuantizer(cfg)
        assert quant is not None

    def test_config_has_fp16_channel_ratio(self):
        from squish.kv.mix_kvq import MixKVQConfig
        cfg = MixKVQConfig()
        assert hasattr(cfg, "fp16_fraction")
        assert 0.0 < cfg.fp16_fraction <= 1.0

    def test_quantize_returns_array(self):
        from squish.kv.mix_kvq import ChannelScorer, MixKVQConfig, MixKVQQuantizer
...
class TestCocktailKVWiring:
    def test_import(self):
        from squish.kv.cocktail_kv import CocktailConfig, CocktailKVStore
        cfg   = CocktailConfig()
        store = CocktailKVStore(cfg)
        assert store is not None

    def test_config_has_chunk_size(self):
        from squish.kv.cocktail_kv import CocktailConfig
        cfg = CocktailConfig()
...
class TestAgileIOWiring:
    def test_import(self):
        from squish.streaming.agile_io import AgileIOConfig, AgileIOManager
    ...
    def test_config_attrs(self):
        from squish.streaming.agile_io import AgileIOConfig
...
class TestMiLoWiring:
    ...
    def test_config_target_bits(self):
        from squish.quant.milo_quant import MiLoConfig
...
class TestSpargeAttnPatchFn:
    def test_patch_model_sparge_attn_importable(self):
        from squish.attention.sparge_attn import patch_model_sparge_attn, unpatch_model_sparge_attn
