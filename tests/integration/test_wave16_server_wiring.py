...
class TestDovetailWiring:
    def test_import(self):
        from squish.speculative.dovetail import DovetailConfig, DovetailCPUVerifier
    ...
    def test_config_defaults(self):
        from squish.speculative.dovetail import DovetailConfig
        cfg = DovetailConfig()
        assert cfg.gamma >= 1
    ...
    def test_decoder_generate_returns_tokens_and_stats(self):
        from squish.speculative.dovetail import (
            DovetailConfig,
            DovetailCPUVerifier,
            DovetailDecoder,
            DovetailDraftRunner,
...
class TestSwiftSpecWiring:
    def test_import(self):
        from squish.speculative.swiftspec import SwiftSpecConfig, SwiftSpecDecoder
    ...
    def test_config_defaults(self):
        from squish.speculative.swiftspec import SwiftSpecConfig
        cfg = SwiftSpecConfig()
        assert cfg.gamma >= 1
...
class TestPIPOWiring:
    def test_import(self):
        from squish.moe.pipo import PIPOConfig, PIPOScheduler
        rng  = np.random.default_rng(0)
        n    = 4
    ...
    def test_config_defaults(self):
        from squish.moe.pipo import PIPOConfig
        cfg = PIPOConfig()
...
class TestMobileMoEWiring:
    def test_import(self):
        from squish.moe.mobile_moe import MoBiLEConfig, MoBiLERouter
    ...
    def test_config_defaults(self):
        from squish.moe.mobile_moe import MoBiLEConfig
        cfg = MoBiLEConfig()
...
class TestOnlineSDWiring:
    def test_import(self):
        from squish.speculative.online_sd import OnlineDraftUpdater, OnlineSDConfig
        cfg     = OnlineSDConfig(buffer_capacity=128, update_every=32)
        updater = OnlineDraftUpdater(cfg)
        assert updater is not None

    def test_config_defaults(self):
        from squish.speculative.online_sd import OnlineSDConfig
        cfg = OnlineSDConfig()
...
class TestLookaheadReasoningWiring:
    ...
    def test_config_defaults(self):
        from squish.token.lookahead_reasoning import LookaheadConfig
        cfg = LookaheadConfig()
...
class TestSparseSpecWiring:
    def test_import(self):
        from squish.speculative.sparse_spec import PillarAttnCache, SparseSpecConfig
    ...
    def test_pillar_cache_update_and_topk(self):
        from squish.speculative.sparse_spec import PillarAttnCache
        rng   = np.random.default_rng(0)
...
class TestFRSpecWiring:
    ...
    def test_calibrate_and_build_subset(self):
        from squish.speculative.fr_spec import FreqTokenSubset, FRSpecCalibrator, FRSpecConfig
        rng = np.random.default_rng(0)
    ...
    def test_frspec_head_compress_expand(self):
        from squish.speculative.fr_spec import FRSpecCalibrator, FRSpecConfig, FRSpecHead
        rng   = np.random.default_rng(2)
        vocab = 32
        hdim  = 8
...
class TestLongSpecWiring:
    def test_import(self):
        from squish.speculative.long_spec import LongSpecConfig, LongSpecHead
    ...
    def test_decoder_generate(self):
        from squish.speculative.long_spec import LongSpecConfig, LongSpecDecoder, LongSpecHead
        rng   = np.random.default_rng(1)
        vocab = 32
        hdim  = 8
...
class TestForelenWiring:
    def test_import(self):
        from squish.token.forelen import EGTPPredictor, ForelenConfig, PLPPredictor
...
class TestRASDWiring:
    def test_import(self):
        from squish.speculative.rasd import CorpusIndex, DraftTree, RASDBatcher, RASDConfig
    ...
    def test_config_defaults(self):
        from squish.speculative.rasd import RASDConfig
        cfg = RASDConfig()
    ...
    def test_corpus_index_add_and_search(self):
        from squish.speculative.rasd import CorpusIndex
        corpus = CorpusIndex(min_prefix_len=2)
    ...
    def test_draft_tree_add_and_traverse(self):
        from squish.speculative.rasd import DraftTree
    ...
    def test_batcher_build_and_prune_tree(self):
        from squish.speculative.rasd import CorpusIndex, RASDBatcher, RASDConfig
