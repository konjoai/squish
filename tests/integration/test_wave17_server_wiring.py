...
class TestSageAttention2Wiring:
    ...
    def test_config_defaults(self):
        from squish.attention.sage_attention2 import SageAttention2Config
    ...
    def test_warp_quantize_function(self):
        from squish.attention.sage_attention2 import warp_quantize_int4
...
class TestStreamingSinkWiring:
    ...
    def test_config_capacity(self):
        from squish.streaming.streaming_sink import SinkConfig
        cfg = SinkConfig(num_sinks=4, window_size=64, head_dim=32)
...
class TestKVSlabWiring:
    def test_import(self):
        from squish.kv.kv_slab import KVPage, KVSlabAllocator
...
class TestSqueezeAttentionWiring:
    ...
    def test_config_defaults(self):
        from squish.attention.squeeze_attention import SqueezeConfig
        cfg = SqueezeConfig()
...
class TestSmallKVWiring:
    ...
    def test_config_defaults(self):
        from squish.kv.smallkv import SmallKVConfig
        cfg = SmallKVConfig()
        assert cfg.n_layers >= 1
...
class TestSpeContextWiring:
    ...
    def test_retrieval_head_top_k(self):
        from squish.speculative.specontext import DistilledRetrievalHead, SpeContextConfig
        rng  = np.random.default_rng(1)
        dim  = 16
        t    = 20
...
class TestSVDqWiring:
    def test_import(self):
        from squish.quant.svdq import SVDqCalibrator, SVDqConfig
    ...
    def test_record_and_search(self):
        from squish.quant.svdq import SVDqCalibrator, SVDqConfig, SVDqPrecisionMap
        rng = np.random.default_rng(0)
    ...
    def test_head_svd_profile(self):
        from squish.quant.svdq import HeadSVDProfile
        rng = np.random.default_rng(2)
...
class TestCommVQWiring:
    ...
    def test_multi_codebook_vq(self):
        from squish.quant.comm_vq import MultiCodebookVQ
        rng  = np.random.default_rng(1)
        dim  = 16
    ...
    def test_fit_kv_codebooks_function(self):
        from squish.quant.comm_vq import CommVQCodebook, fit_kv_codebooks
        rng  = np.random.default_rng(2)
...
class TestGemFilterWiring:
    ...
    def test_attention_score_buffer(self):
        from squish.token.gemfilter import AttentionScoreBuffer, GemFilterConfig
        rng    = np.random.default_rng(1)
...
class TestMInferencePatchWiring:
    ...
    def test_select_pattern_short_sequence(self):
        from squish.attention.minference_patch import select_pattern_for_sequence
...
class TestPromptCompressorWiring:
    def test_import(self):
        from squish.context.prompt_compressor import compress
...
class TestTrailWiring:
    def test_import(self):
        from squish.speculative.trail import TrailConfig, TrailLinearProbe, TrailPredictor
    ...
    def test_probe_fit_and_predict(self):
        from squish.speculative.trail import TrailConfig, TrailLinearProbe
        rng    = np.random.default_rng(0)
        hdim   = 32
    ...
    def test_trail_stats_record(self):
        from squish.speculative.trail import TrailStats
        stats = TrailStats()
