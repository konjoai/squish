...
class TestTreeVerifierWiring:
    ...
    def test_config_defaults(self):
        from squish.speculative.tree_verifier import VerifyConfig
        cfg = VerifyConfig()
...
class TestKVCompressWiring:
    ...
    def test_config_defaults(self):
        from squish.kv.kv_compress import KVCompressConfig
...
class TestDynamicNTKWiring:
    ...
    def test_config_defaults(self):
        from squish.attention.dynamic_ntk import DynamicNTKConfig
    ...
    def test_get_freqs_shape(self):
        from squish.attention.dynamic_ntk import DynamicNTKConfig, DynamicNTKScaler
...
class TestQuantSpecDecodeWiring:
    ...
    def test_config_defaults(self):
        from squish.speculative.quant_spec_decode import QSDConfig
        cfg = QSDConfig()
        assert cfg.n_draft_tokens >= 1
...
class TestSparseAttnIndexWiring:
    ...
    def test_config_defaults(self):
        from squish.attention.sparse_attn_index import IndexConfig
        cfg = IndexConfig()
        assert cfg.top_k >= 1
        assert cfg.head_dim >= 1
        assert cfg.n_heads >= 1
    ...
    def test_stats(self):
        from squish.attention.sparse_attn_index import IndexConfig, IndexStats, SparseAttnIndex
        rng = np.random.default_rng(2)
...
class TestPipelineBubbleWiring:
    ...
    def test_config_defaults(self):
        from squish.hardware.pipeline_bubble import StageConfig
        cfg = StageConfig()
...
class TestLayerwiseDecodeWiring:
    ...
    def test_config_defaults(self):
        from squish.token.layerwise_decode import LayerwiseConfig
        cfg = LayerwiseConfig()
...
class TestCodecKVWiring:
    ...
    def test_config_defaults(self):
        from squish.kv.codec_kv import CodecConfig
        cfg = CodecConfig()
    ...
    def test_fit_and_encode_decode(self):
        from squish.kv.codec_kv import CodecConfig, KVCodec
        rng = np.random.default_rng(0)
...
class TestDedupeAttnWiring:
    ...
    def test_config_defaults(self):
        from squish.attention.dedupe_attn import DedupConfig
        cfg = DedupConfig()
...
class TestBudgetSpecWiring:
    ...
    def test_config_defaults(self):
        from squish.serving.budget_spec import BudgetConfig
        cfg = BudgetConfig()
...
class TestRetentionAttnWiring:
    ...
    def test_config_defaults(self):
        from squish.attention.retention_attn import RetentionConfig
        cfg = RetentionConfig()
        assert cfg.hidden_dim >= 1
        assert cfg.n_heads >= 1
    ...
    def test_stats_and_reset(self):
        from squish.attention.retention_attn import RetentionConfig, RetentionKernel, RetentionStats
        rng = np.random.default_rng(1)
