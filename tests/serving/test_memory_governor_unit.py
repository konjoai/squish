...
class TestReadVmStat:
    def test_parses_available_gb(self):
        with patch("squish.serving.memory_governor._run", return_value=_SAMPLE_VM_STAT):
...
"""
        with patch("squish.serving.memory_governor._run", return_value=vm_stat_no_page_size):
            avail, wired = _read_vm_stat()
...
def _patched_governor(poll_interval=60.0, pressure=0, avail=8.0, wired=2.0):
    ...
    with patch("squish.serving.memory_governor._read_pressure_level", return_value=pressure), \
         patch("squish.serving.memory_governor._read_vm_stat", return_value=(avail, wired)):
...
class TestMemoryGovernorLifecycle:
    def test_start_returns_self(self):
        with patch("squish.serving.memory_governor._read_pressure_level", return_value=0), \
             patch("squish.serving.memory_governor._read_vm_stat", return_value=(8.0, 1.0)):
...
class TestMemoryGovernorCallbacks:
    def test_callback_called_on_pressure_change(self):
        ...
        with patch("squish.serving.memory_governor._read_pressure_level", return_value=LEVEL_WARNING), \
             patch("squish.serving.memory_governor._read_vm_stat", return_value=(4.0, 2.0)):
            gov._poll_once()
        gov.stop()
    ...
    def test_callback_not_called_without_change(self):
        ...
        with patch("squish.serving.memory_governor._read_pressure_level", return_value=LEVEL_NORMAL), \
             patch("squish.serving.memory_governor._read_vm_stat", return_value=(8.0, 1.0)):
            gov._poll_once()
        gov.stop()
        assert len(fired) == 0
